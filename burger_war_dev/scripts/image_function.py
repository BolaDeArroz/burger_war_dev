...
def detect_field_trap(frame):
    ...
    # hsv real world
    rnb_yellow = createMaskImage(hsv, [28 , 50], [100, 255], [100, 255])
    
    # cv2.imshow("Hue", hsv)
    # cv2.imshow("img", img)
    # cv2.imshow("rnb_yellow", rnb_yellow)
    

    # METHOD1: fill hole in the object using Closing process (Dilation next to Erosion) of mathematical morphology
    #rnb_yellow = cv2.morphologyEx(rnb_yellow, cv2.MORPH_OPEN, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(25,25)))
    #cv2.imshow("2", rnb_yellow)
    rnb_yellow = cv2.morphologyEx(rnb_yellow, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(14,14)))
    """
